...
// @Router /zulu/api/v2/points/{elem_id}/categories/{category_id} [get]
...
func (f *facade) GetPointCategoryDataV2(c *gin.Context) {
	...
	if groupType == "group" {
		...
		measurementsToRequest := make(map[string]core.GetPointsDataRequestMeasurement)
		...
		for _, packetMap := range zuluFullPoint.Measurements {
			for groupID, measurements := range packetMap {
				...
				if measurements.Data.In != "" || measurements.Data.Out != "" {
					measurementsToRequest[groupIDstring] = core.GetPointsDataRequestMeasurement{
		...
		coreRequest := []core.GetPointsDataRequest{}
		coreRequest = append(coreRequest, core.GetPointsDataRequest{
			ElemID:       elementIDInt,
			Measurements: measurementsToRequest,
		})

		coreResponses, err := f.coreDataApiClient.GetPointsData(coreRequest, timestamp)
		if err != nil {
		...
		var ourData core.GetPointsDataResponse
		if len(coreResponses) > 0 {
			ourData = coreResponses[0]
		} else {
			ourData = core.GetPointsDataResponse{}
		}
		...
		for _, measurementsMap := range tempData {
			for groupID, tempMeasurement := range *measurementsMap {
				...
				if existing, exists := mergedMeasurements[groupID]; exists {
					...
					data := ourData.Measurements[groupID]
					mergedMeasurements[groupID] = GroupMeasurement{
						ID: groupID,
						Rn: tempMeasurement.Rn,
						Data: GroupMeasurementsData{
							In:  multiplyPointers(data.I, coeff.lersCoeff),
							Out: multiplyPointers(data.O, coeff.lersCoeff),
						},
						CalculatedData: GroupMeasurementsCalculatedData{
							In:  multiplyPointers(tempMeasurement.CalculatedData.In, coeff.zuluCoeff),
							Out: multiplyPointers(tempMeasurement.CalculatedData.Out, coeff.zuluCoeff),
		...
		fullPoint = GetPointDataByCategoryGroup{
			Measurements: make([]GroupMeasurement, 0, len(mergedMeasurements)), IsDataCopied: ourData.IsDataCopied, IsCalculatedDataCopied: isCalculatedDataCopied,
		}
